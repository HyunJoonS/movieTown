...
function Board_Modify({state}) {
    ...
    useEffect(()=>{
      setSelected(state.boardType);
      setTitle(state.boardTitle);
      setValue(state.boardText);
    },[])
    ...
    const onSubmit = ()=>{
      ...
      if(body.text && body.title && selected != 'none'){
        axios.post('/api/board/updqte',body).then((res)=>{
          alert(res.data);
          history.push(`/board/${body.bbsType}/${state.boardID}`)
        })
        console.log(body);
      }else{
        alert('입력 내용을 확인해주세요')
    ...
    return(
        <div className="Board_Write">
            ...
            <div className="boardTitle">
              <input type='text' value={title} placeholder='제목을 입력해 주세요.' className='form'
